...
function calculate(shifts:Shift[],memberId:string,settings:Settings,from:string,to:string){
  ...
  for(const shift of rows){
    ...
    const defaultBreak=segments.some(([a,b])=>b>Math.max(a,17*60))?settings.dinnerBreak:0;
    const breakMinutes=settings.breakOverrides[shift.id]??defaultBreak;
    const total=Math.max(0,segments.reduce((sum,[a,b])=>sum+b-a,0)-breakMinutes);
...
export default function PayrollSettings({member,shifts,onClose}:{member:Member;shifts:Shift[];onClose:()=>void}){
  ...
  return <div className="payroll-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="payroll-panel">
    ...
    <div className="payroll-form">
      ...
      <label>ディナーの標準休憩（分）<input type="text" inputMode="numeric" pattern="[0-9]*" value={breakInput} onFocus={e=>e.currentTarget.select()} onChange={e=>{const value=e.target.value.replace(/\D/g,'').slice(0,4);setBreakInput(value);if(value!=='')change('dinnerBreak',Number(value));else setSaved(false)}} onBlur={()=>{if(breakInput===''){setBreakInput('20');change('dinnerBreak',20)}}}/></label>
    ...
    {result.rows.length>0&&<div className="payroll-shift-breaks"><div className="payroll-shift-breaks-heading"><b>勤務別の休憩時間</b><small>休憩なしの日は0分に変更できます</small></div>{result.rows.map(shift=>{const segments=paidSegments(shift,settings.excludeClosedGap),defaultBreak=segments.some(([a,b])=>b>Math.max(a,17*60))?settings.dinnerBreak:0,value=settings.breakOverrides[shift.id]??defaultBreak;return <label key={shift.id}><span><b>{new Date(`${shift.shift_date}T00:00:00`).toLocaleDateString('ja-JP',{month:'numeric',day:'numeric',weekday:'short'})}</b><small>{shift.start_time.slice(0,5)}〜{shift.end_time.slice(0,5)}</small></span><span className="payroll-break-input"><input aria-label={`${shift.shift_date}の休憩時間`} type="text" inputMode="numeric" pattern="[0-9]*" value={value} onFocus={e=>e.currentTarget.select()} onChange={e=>{const minutes=Math.max(0,Number(e.target.value.replace(/\D/g,'').slice(0,3)||0));change('breakOverrides',{...settings.breakOverrides,[shift.id]:minutes})}}/><em>分</em></span></label>})}</div>}
