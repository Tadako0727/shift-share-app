...
type Settings={hourly:number;latePremium:number;transport:number;dinnerBreak:number;breakOverrides:Record<string,number>;excludeClosedGap:boolean;rounding:1|15|30;closingDay:number;payday:number;payLag:0|1};
const defaults:Settings={hourly:1200,latePremium:25,transport:0,dinnerBreak:20,breakOverrides:{},excludeClosedGap:true,rounding:1,closingDay:31,payday:25,payLag:1};
...
const loadSettings=(id:string):Settings=>{try{const saved=JSON.parse(localStorage.getItem(storageKey(id))||'{}');return {...defaults,...saved,dinnerBreak:saved.dinnerBreak??20,breakOverrides:saved.breakOverrides??{}}}catch{return defaults}};
...
function paidSegments(shift:Shift,_excludeGap:boolean):[number,number][]{
...
function calculate(shifts:Shift[],memberId:string,settings:Settings,from:string,to:string){
  ...
  let regular=0,late=0,segmentCount=0;
  for(const shift of rows){
    const segments=paidSegments(shift,settings.excludeClosedGap);
    for(const [start,end] of segments){
      const isDinner=end>Math.max(start,17*60);
      const breakMinutes=isDinner?(settings.breakOverrides[shift.id]??settings.dinnerBreak):0;
      const total=Math.max(0,end-start-breakMinutes);
      const rounded=Math.floor(total/settings.rounding)*settings.rounding;
      const lateRaw=Math.max(0,end-Math.max(start,22*60));
      const lateRounded=Math.min(rounded,lateRaw);
      regular+=rounded-lateRounded;late+=lateRounded;segmentCount+=1;
    }
  }
  ...
  return {rows,segmentCount,regular,late,base,premium,transport,total:base+premium+transport};
}
...
export default function PayrollSettings({member,shifts,onClose}:{member:Member;shifts:Shift[];onClose:()=>void}){
  ...
  const [breakInput,setBreakInput]=useState(()=>String(loadSettings(member.id).dinnerBreak));
  ...
  const save=()=>{const hourly=Math.max(0,Number(hourlyInput||0)),latePremium=Math.max(0,Number(premiumInput||0)),dinnerBreak=Math.max(0,Number(breakInput||0)),transport=Math.max(0,Number(transportInput||0));const next={...settings,hourly,latePremium,dinnerBreak,transport};setSettings(next);setHourlyInput(String(hourly));setPremiumInput(String(latePremium));setBreakInput(String(dinnerBreak));setTransportInput(String(transport));localStorage.setItem(storageKey(member.id),JSON.stringify(next));setSaved(true)};
  return <div className="payroll-backdrop" onMouseDown={e=>e.target===e.currentTarget&&onClose()}><section className="payroll-panel">
    <header><div><h2><Calculator/>給与計算</h2><p>{shownName(member)}さんの端末内だけに保存します。</p></div><button onClick={onClose}><X/></button></header>
    <div className="payroll-next"><small>次の給料日 · {nextPay.payDate.toLocaleDateString('ja-JP',{month:'long',day:'numeric',weekday:'short'})}</small><strong>{money(nextPay.result.total)}</strong><span>{nextPay.periodStart.toLocaleDateString('ja-JP')}〜{nextPay.periodEnd.toLocaleDateString('ja-JP')} · {nextPay.result.rows.length}日出勤・{nextPay.result.segmentCount}勤務枠</span></div>
    ...
    <div className="payroll-total"><small>確定シフトからの概算</small><strong>{money(result.total)}</strong><span>{result.rows.length}日出勤・{result.segmentCount}勤務枠・{((result.regular+result.late)/60).toFixed(1)}時間</span></div>
    ...
    <div className="payroll-form">
      ...
      <label>ディナーの平均休憩時間（分）<input type="text" inputMode="numeric" pattern="[0-9]*" value={breakInput} onFocus={e=>e.currentTarget.select()} onChange={e=>{const value=e.target.value.replace(/\D/g,'').slice(0,4);setBreakInput(value);if(value!=='')change('dinnerBreak',Number(value));else setSaved(false)}} onBlur={()=>{if(breakInput===''){setBreakInput('20');change('dinnerBreak',20)}}}/></label>
      ...
      <div className="payroll-check"><span><b>通し勤務はランチ・ディナーに分割</b><small>15:00〜17:00は勤務時間から除外し、それぞれ別の勤務枠として計算します。</small></span></div>
    </div>
    {result.rows.some(shift=>paidSegments(shift,settings.excludeClosedGap).some(([a,b])=>b>Math.max(a,17*60)))&&<div className="payroll-shift-breaks"><div className="payroll-shift-breaks-heading"><b>勤務別のディナー休憩</b><small>休憩なしの日は0分に変更できます。ランチは休憩0分です。</small></div>{result.rows.filter(shift=>paidSegments(shift,settings.excludeClosedGap).some(([a,b])=>b>Math.max(a,17*60))).map(shift=>{const value=settings.breakOverrides[shift.id]??settings.dinnerBreak;return <label key={shift.id}><span><b>{new Date(`${shift.shift_date}T00:00:00`).toLocaleDateString('ja-JP',{month:'numeric',day:'numeric',weekday:'short'})}</b><small>{shift.start_time.slice(0,5)}〜{shift.end_time.slice(0,5)}</small></span><span className="payroll-break-input"><input aria-label={`${shift.shift_date}のディナー休憩時間`} type="text" inputMode="numeric" pattern="[0-9]*" value={value} onFocus={e=>e.currentTarget.select()} onChange={e=>{const minutes=Math.max(0,Number(e.target.value.replace(/\D/g,'').slice(0,3)||0));change('breakOverrides',{...settings.breakOverrides,[shift.id]:minutes})}}/><em>分</em></span></label>})}</div>}
